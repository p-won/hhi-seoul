...
# ── 4. 행정동·자치구·프랜차이즈비율_평균 추출 ─────────────────────────────────
franchise_slim = franchise_df[["자치구", "행정동", "프랜차이즈비율_평균"]].copy()
...
# 자치구코드(행정동코드 앞 5자리) → 자치구명 매핑 (동명 중복 해소용)
# 크롤링 데이터의 자치구명과 일치시켜 (자치구명, 행정동명) 복합키로 병합
GU_CODE_MAP = {
...
v2_df["_자치구"] = v2_df["행정동코드"].apply(lambda c: GU_CODE_MAP.get(int(str(c)[:5])))

DONG_ALIAS = {
    "상일1동": "상일동",
    "상일2동": "상일동",
    # 행정동 개편: 일원2동 → 개포3동 (크롤링 데이터는 구코드명 기준)
    "개포3동": "일원2동",
...
franchise_slim["_행정동_key"] = (
    franchise_slim["행정동"].apply(normalize_dong).str.replace(".", "·", regex=False)
)
franchise_slim["_자치구"] = franchise_slim["자치구"]

v3_df = v2_df.merge(
    franchise_slim[["_자치구", "_행정동_key", "프랜차이즈비율_평균"]],
    on=["_자치구", "_행정동_key"],
    how="left"
).drop(columns=["_행정동_key", "_자치구"])
